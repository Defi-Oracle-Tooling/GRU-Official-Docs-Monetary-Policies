...
contract MonetaryFacet is IMonetary {
    ...
    function mintM1(address to, uint256 amount) external onlyMonetaryRole nonReentrant { to; amount; revert Errors.ErrNotReady(); }
    function burnM1(address from, uint256 amount) external onlyMonetaryRole nonReentrant { from; amount; revert Errors.ErrNotReady(); }
    function issueM0(address to, uint256 amount) external onlyMonetaryRole nonReentrant { to; amount; revert Errors.ErrNotReady(); }
    function redeemM0(address from, uint256 amount) external onlyMonetaryRole nonReentrant { from; amount; revert Errors.ErrNotReady(); }
    function setScalarS(uint256 newScalar) external onlyMonetaryRole nonReentrant {
