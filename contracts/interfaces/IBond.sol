...
interface IBond {
    event BondIssued(bytes32 series, address indexed to, uint256 notional);
    event CouponsAccrued(bytes32 series, uint256 amount);
    event BondBuyback(bytes32 series, uint256 principalClosed);
